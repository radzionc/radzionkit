// This file is generated by @reactkit/ui/country/codegen/generateFlags.ts
import { SVGProps } from 'react'

const CmFlag = (props: SVGProps<SVGSVGElement>) => (
  <svg
    {...props}
    width="1em"
    height="0.75em"
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    id="flag-icons-cm"
    viewBox="0 0 640 480"
    {...props}
  >
    <path fill="#007a5e" d="M0 0h213.3v480H0z" />
    <path fill="#ce1126" d="M213.3 0h213.4v480H213.3z" />
    <path fill="#fcd116" d="M426.7 0H640v480H426.7z" />
    <g fill="#fcd116" transform="translate(320 240) scale(7.1111)">
      <g id="cm-b">
        <path id="cm-a" d="M0-8-2.5-.4 1.3.9z" />
        <use xlinkHref="#cm-a" transform="scale(-1 1)" />
      </g>
      <use xlinkHref="#cm-b" transform="rotate(72)" />
      <use xlinkHref="#cm-b" transform="rotate(144)" />
      <use xlinkHref="#cm-b" transform="rotate(-144)" />
      <use xlinkHref="#cm-b" transform="rotate(-72)" />
    </g>
  </svg>
)

export default CmFlag
